...
warningPanel.port.on("re-enable-form", function() {
  ...
  if(haveBrowserSupport) {
    redScriptWorkerPort("re-enable-form");
  }
  else{
...
pageMod.PageMod({
    ...
    onAttach: function(worker) {
      redScriptWorkerPort = worker.port.emit;
      if(shouldColorRed)
      {
...
function passwordDetectedListener(evnt) {
  ...
  if ("form" == evnt.data) {
    notification = evnt.subject.QueryInterface(Ci.nsIDOMHTMLFormElement);
    offenderURL = notification.action;
  }
  else if ("page" == evnt.data) {
    notification = evnt.subject.QueryInterface(Ci.nsIDOMDocument);
    offenderURL = notification.defaultView.top.location.href;
  }
  else if ("iframe" == evnt.data) {
    notification = evnt.subject.QueryInterface(Ci.nsIDOMDocument);
    offenderURL = notification.URL;
  }

  // change the notification object that we got from the browser
  // into our lighter object that we send around
  decision = new DecisionResult(evnt.data, offenderURL);
